...
parser.add_argument('--name', type=str, help='exp name', default='same-vanilla-3.21')
...
if training:
    ...
    CVM_model = CVM(device, circular_padding, mask=False)
    CVM_model.to(device)
    ...
    for epoch in range(15):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader, 0):
            grd, sat, gt, gt_with_ori, gt_orientation, city, _, _ = data
            grd = grd.to(device)
            sat = sat.to(device)
            gt = gt.to(device)
        ...
        torch.save(CVM_model.cpu().state_dict(), model_dir+f'{name}.pt')  # saving model
    ...
    test_model_path = '/data/test/code/CCVPE/models/VIGOR/crossarea_HFoV360/5/cross-aug-3.20.pt'
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataloader, 0):
            # print(i)
            grd, sat, gt, gt_with_ori, gt_orientation, city, orientation_angle, _ = data
            grd = grd.to(device)
            sat = sat.to(device)
