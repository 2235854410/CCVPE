...
from SLR import add_extra_weights, ScaledLowRankConfigTimmViT, ScaledLowRankAdapter, ScaledLowRankConvAdapter
...
class CVM_VIGOR_with_foundation_model(nn.Module):
    def __init__(self, device, circular_padding, backbone):
        super(CVM_VIGOR_with_foundation_model, self).__init__()
        self.device = device
        ...
        if backbone == "efficientnet":
            ...
            self.sat_dino = add_extra_weights(
                DINO(dino_name="dinov2"),
                config,
            ...
            self.grd_dino = add_extra_weights(
                DINO(dino_name="dinov2"),
                config,
                ScaledLowRankAdapter,
                ScaledLowRankConvAdapter,
        ...
        self.sat_feature_to_descriptors = nn.Sequential(nn.Flatten(start_dim=1),
                                                        nn.Linear(768 * 2 * 2, 768)
                                                        )

        self.sat_normalization = normalization(2, 1)

        # loc
        self.deconv6 = nn.ConvTranspose2d(769, 512, 2, 2)
        self.conv6 = double_conv(1280, 640)

        self.deconv5 = nn.ConvTranspose2d(641, 320, 2, 2)
        self.conv5 = double_conv(1088, 320)

        self.deconv4 = nn.ConvTranspose2d(321, 160, 2, 2)
        self.conv4 = double_conv(928, 160)

        self.deconv3 = nn.ConvTranspose2d(161, 80, 2, 2)
        self.conv3 = double_conv(848, 80)

        self.deconv2 = nn.ConvTranspose2d(81, 40, 2, 2)
        self.conv2 = double_conv(808, 40)
        ...
        # ori
        self.deconv6_ori = nn.ConvTranspose2d(788, 512, 2, 2)
        self.conv6_ori = double_conv(1280, 640)

        self.deconv5_ori = nn.ConvTranspose2d(640, 256, 2, 2)
        self.conv5_ori = double_conv(1024, 256)

        self.deconv4_ori = nn.ConvTranspose2d(256, 128, 2, 2)
        self.conv4_ori = double_conv(896, 128)

        self.deconv3_ori = nn.ConvTranspose2d(128, 64, 2, 2)
        self.conv3_ori = double_conv(832, 64)

        self.deconv2_ori = nn.ConvTranspose2d(64, 32, 2, 2)
        self.conv2_ori = double_conv(800, 32)
    ...
    def forward(self, grd, sat):
        multiscale_grd = self.grd_dino(grd)  # shape=[8, 768, 23, 46]
        multiscale_sat = self.sat_dino(sat) # shape=[8, 768, 37, 37]
        # print(multiscale_sat[0].shape)
        # print(multiscale_grd[0].shape)
        grd_feature_volume = F.interpolate(multiscale_grd[-1], size=(10, 20), mode="bilinear", align_corners=False)
        sat_feature_volume = F.interpolate(multiscale_sat[4], size=(16, 16), mode="bilinear", align_corners=False)
        ...
        sat_feature_block0 = F.interpolate(multiscale_sat[0], size=(256, 256), mode="bilinear", align_corners=False)   # [16, 256, 256]
        sat_feature_block2 = F.interpolate(multiscale_sat[1], size=(128, 128), mode="bilinear", align_corners=False)  # [24, 128, 128]
        sat_feature_block4 = F.interpolate(multiscale_sat[2], size=(64, 64), mode="bilinear", align_corners=False)  # [40, 64, 64]
        sat_feature_block10 = F.interpolate(multiscale_sat[3], size=(32, 32), mode="bilinear", align_corners=False)  # [112, 32, 32]
        sat_feature_block15 = F.interpolate(multiscale_sat[4], size=(16, 16), mode="bilinear", align_corners=False) # [320, 16, 16]
        ...
        for i in range(20):
            ...
            # print(f"grd_descriptor_map1.shape = {grd_descriptor_map1.shape}")
            # print(f"sat_descriptor_map_window.shape = {sat_descriptor_map_window.shape}")

            matching_score = torch.sum((grd_descriptor_map1 * sat_descriptor_map_window), dim=1, keepdim=True) / (
        ...
        for i in range(20):
            ...
            matching_score = torch.sum((grd_descriptor_map2 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
        ...
        for i in range(20):
            ...
            matching_score = torch.sum((grd_descriptor_map3 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
        ...
        for i in range(20):
            ...
            matching_score = torch.sum((grd_descriptor_map4 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
        ...
        for i in range(20):
            ...
            matching_score = torch.sum((grd_descriptor_map5 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
        ...
        x = self.deconv2(x)
        # print(x.shape)
        # print(sat_feature_block0.shape)
        ...
        for i in range(20):
            ...
            matching_score = torch.sum((grd_descriptor_map6 * sat_descriptor_map_window), dim=1, keepdim=True) / (
                        sat_map_norm * grd_map_norm)  # cosine similarity
            if i == 0:
        ...
        # ori
        x_ori = torch.cat([matching_score_stacked, self.sat_normalization(sat_descriptor_map)],
                          dim=1)  # [8, 1300, 8, 8]
