...
parser.add_argument('--name', type=str, help='h', default='distill')
...
parser.add_argument('--model_path', type=str, help='h', default='/data/test/code/CCVPE/ckpt/kitti.pt')
...
if training:
    ...
    with torch.no_grad():
        for i, data in enumerate(test1_loader, 0):
            ...
            for batch_idx in range(gt.shape[0]):
                ...
                gt2pred_from_north = np.arctan2(np.abs(loc_gt[2] - loc_pred[2]),
                                                np.abs(loc_gt[1] - loc_pred[1])) * 180 / math.pi  # degree
    ...
    print('percentage of samples with lateral localization error under 1m, 3m, and 5m: ',
          ...
          np.sum(lateral_error_in_meters < 5) / len(lateral_error_in_meters))
    print('percentage of samples with longitudinal localization error under 1m, 3m, and 5m: ',
          np.sum(longitudinal_error_in_meters < 1) / len(longitudinal_error_in_meters),
          np.sum(longitudinal_error_in_meters < 3) / len(longitudinal_error_in_meters),
          np.sum(longitudinal_error_in_meters < 5) / len(longitudinal_error_in_meters))
    print('percentage of samples with orientation error under 1 degree, 3 degrees, and 5 degrees: ',
          np.sum(orientation_error < 1) / len(orientation_error),
          np.sum(orientation_error < 3) / len(orientation_error),
          np.sum(orientation_error < 5) / len(orientation_error))
